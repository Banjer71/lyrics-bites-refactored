...
const SongPage = (props) => {
	...
	useEffect(
		() => {
			...
			fetch(`https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${trackId}&apikey=@musicmatch`)
				...
				.then((data) => {
					...
					return fetch(
						`https://api.musixmatch.com/ws/1.1/track.search?q_track=${songTrack}&apikey=@musicmatch`
					)
						.then((res) => res.json())
						.then((data) => {
							...
							return fetch(
								`https://api.musixmatch.com/ws/1.1/album.tracks.get?album_id=@musicmatch`
							)
								.then((res) => res.json())
								.then((data) => {
	...
	useEffect(
		() => {
			...
			fetch(
				`http://ws.audioscrobbler.com/2.0/?method=album.search&album=${album}&api_key=@lastfm&format=json`,
				{ signal: signal }
			)
				.then((res) => res.json())
				.then((data) => {
	...
	const getAlbumTracks = (idTrack, idAlbum) => {
		fetch(`https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${idTrack}&apikey=@musicmatch`)
			.then((res) => res.json())
			.then((data) => {
				...
				return fetch(
					`https://api.musixmatch.com/ws/1.1/album.tracks.get?album_id=${idAlbum}&apikey=$@musicmatch`
				)
					.then((res) => res.json())
					.then((data) => {
						const songName = data.message.body.track_list;
